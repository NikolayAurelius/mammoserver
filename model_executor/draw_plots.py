...
def draw_plots(fname):
   ...
   # 	filename = f'vizualizations/images/{fname}/3dplot', euclid_colors = True)
	...
	plt.savefig(IMGSDIR + f'/{fname}/matrix_voltage_error.jpg')
	plt.close()
	...
	for act in ['l', 'g']:
		for i in range(18):
			for j in range(18):
				...
				plt.title('slice')
				plt.savefig(IMGSDIR + f'/{fname}/slice{i}_{j}_{act}.jpg')
				plt.close()

				fig = plt.figure(figsize=(5,5))
				plt.imshow(img2, cmap = 'hot')
				plt.title('deviance')
				plt.savefig(IMGSDIR + f'/{fname}/deviance{i}_{j}_{act}.jpg')
				plt.close()

				fig = plt.figure(figsize=(5,5))
				plt.imshow(img3, cmap = 'hot')
				plt.title('meas')
				plt.savefig(IMGSDIR + f'/{fname}/meas{i}_{j}_{act}.jpg')	
				plt.close()

	IMGSNAMES = os.listdir(IMGSDIR + f'/{fname}')
	with ZipFile(IMGSDIR + f'/{fname}/{fname[:-4]}.zip', 'w') as zipObj:
		for imgname in IMGSNAMES:
			zipObj.write(IMGSDIR + f'/{fname}/{imgname}', fname + '/' + imgname)


def isnew():

	list_of_files = os.listdir(IMGSDIR)
